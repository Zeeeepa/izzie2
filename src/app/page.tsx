'use client';

import { authClient } from '@/lib/auth-client';
import { useRouter, useSearchParams } from 'next/navigation';
import { useState, useEffect, Suspense } from 'react';
import Link from 'next/link';

/**
 * Invite code requirement check
 * Set NEXT_PUBLIC_REQUIRE_INVITE_CODE=true to require invite codes
 */
const REQUIRE_INVITE_CODE = process.env.NEXT_PUBLIC_REQUIRE_INVITE_CODE === 'true';

/**
 * Landing Page Content Component (needs Suspense wrapper for useSearchParams)
 */
function HomePageContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [isCheckingSession, setIsCheckingSession] = useState(true);

  // Invite code state
  const [inviteCode, setInviteCode] = useState('');
  const [inviteCodeValid, setInviteCodeValid] = useState<boolean | null>(null);
  const [inviteCodeMessage, setInviteCodeMessage] = useState<string | null>(null);
  const [isValidatingCode, setIsValidatingCode] = useState(false);

  // Check if already logged in and redirect
  useEffect(() => {
    authClient.getSession().then(async (result) => {
      if (result.data?.user) {
        // Check if this is a new signup that needs to use an invite code
        const pendingInviteCode = localStorage.getItem('pendingInviteCode');
        if (pendingInviteCode && REQUIRE_INVITE_CODE) {
          try {
            await fetch('/api/invite/use', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ code: pendingInviteCode }),
            });
            localStorage.removeItem('pendingInviteCode');
          } catch (error) {
            console.error('Failed to consume invite code:', error);
          }
        }
        router.push('/dashboard');
      } else {
        setIsCheckingSession(false);

        // Check for error in URL params (e.g., from failed OAuth)
        const errorParam = searchParams.get('error');
        if (errorParam) {
          setError(decodeURIComponent(errorParam));
        }

        // Restore invite code from localStorage if present
        const savedCode = localStorage.getItem('pendingInviteCode');
        if (savedCode) {
          setInviteCode(savedCode);
          setInviteCodeValid(true); // Assume it was previously validated
        }
      }
    });
  }, [router, searchParams]);

  // Validate invite code
  const validateInviteCode = async (code: string) => {
    if (!code.trim()) {
      setInviteCodeValid(null);
      setInviteCodeMessage(null);
      return;
    }

    setIsValidatingCode(true);
    try {
      const response = await fetch('/api/invite/validate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: code.trim() }),
      });
      const data = await response.json();
      setInviteCodeValid(data.valid);
      setInviteCodeMessage(data.message);

      // Store valid code for use after OAuth
      if (data.valid) {
        localStorage.setItem('pendingInviteCode', code.trim().toUpperCase());
      } else {
        localStorage.removeItem('pendingInviteCode');
      }
    } catch (error) {
      setInviteCodeValid(false);
      setInviteCodeMessage('Failed to validate code');
    } finally {
      setIsValidatingCode(false);
    }
  };

  // Debounced validation on input change
  useEffect(() => {
    const timer = setTimeout(() => {
      if (inviteCode.trim().length >= 6) {
        validateInviteCode(inviteCode);
      } else if (inviteCode.trim().length === 0) {
        setInviteCodeValid(null);
        setInviteCodeMessage(null);
        localStorage.removeItem('pendingInviteCode');
      }
    }, 500);

    return () => clearTimeout(timer);
  }, [inviteCode]);

  const handleGoogleSignIn = async () => {
    // Check invite code requirement
    if (REQUIRE_INVITE_CODE && !inviteCodeValid) {
      setError('Please enter a valid invite code to continue');
      return;
    }

    setIsLoading(true);
    setError(null);

    try {
      const result = await authClient.signIn.social({
        provider: 'google',
        callbackURL: '/dashboard',
      });

      // If signIn returns a redirect URL, navigate to it
      const resultData = result?.data as { url?: string } | null;
      const resultAny = result as { url?: string; redirect?: string } | null;
      if (resultData?.url) {
        window.location.href = resultData.url;
      } else if (resultAny?.url) {
        window.location.href = resultAny.url;
      } else if (resultAny?.redirect) {
        window.location.href = resultAny.redirect;
      }
    } catch (err) {
      console.error('Sign in error:', err);
      setError(err instanceof Error ? err.message : 'Sign in failed');
      setIsLoading(false);
    }
  };

  // Show minimal loading state while checking session
  if (isCheckingSession) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-blue-50">
        <div className="animate-pulse">
          <div className="h-8 w-32 bg-slate-200 rounded" />
        </div>
      </div>
    );
  }

  return (
    <main className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden">
      {/* Background with subtle gradient and pattern */}
      <div className="absolute inset-0 bg-gradient-to-br from-slate-50 via-white to-blue-50" />

      {/* Subtle dot pattern overlay */}
      <div
        className="absolute inset-0 opacity-[0.4]"
        style={{
          backgroundImage: `radial-gradient(circle at 1px 1px, rgb(148 163 184 / 0.3) 1px, transparent 0)`,
          backgroundSize: '24px 24px',
        }}
      />

      {/* Decorative gradient orbs */}
      <div className="absolute top-1/4 -left-32 w-96 h-96 bg-blue-400/20 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 -right-32 w-96 h-96 bg-indigo-400/20 rounded-full blur-3xl" />

      {/* Content */}
      <div className="relative z-10 flex flex-col items-center px-6 py-12 w-full max-w-md">
        {/* Logo/Brand */}
        <div className="mb-8 text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 mb-6 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-2xl shadow-lg shadow-blue-500/25">
            <svg
              className="w-8 h-8 text-white"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              strokeWidth={2}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
              />
            </svg>
          </div>
          <h1 className="text-4xl font-bold tracking-tight text-slate-900">
            Izzie
          </h1>
          <p className="mt-3 text-lg text-slate-600">
            Your AI-powered email intelligence
          </p>
        </div>

        {/* Feature highlights */}
        <div className="w-full mb-8 space-y-3">
          <div className="flex items-start gap-3 text-sm text-slate-600">
            <svg className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span>Learns from your emails and calendar</span>
          </div>
          <div className="flex items-start gap-3 text-sm text-slate-600">
            <svg className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span>Extracts people, companies, projects, and relationships</span>
          </div>
          <div className="flex items-start gap-3 text-sm text-slate-600">
            <svg className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span>Creates action items and syncs to Google Tasks</span>
          </div>
          <div className="flex items-start gap-3 text-sm text-slate-600">
            <svg className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span>Intelligent chat with full context awareness</span>
          </div>
          <div className="flex items-start gap-3 text-sm text-slate-600">
            <svg className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            <span>Discovers patterns in your professional network</span>
          </div>
        </div>

        {/* Error message */}
        {error && (
          <div className="w-full mb-6 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm">
            {error}
          </div>
        )}

        {/* Invite Code Input - shown when invite codes are required */}
        {REQUIRE_INVITE_CODE && (
          <div className="w-full mb-6">
            <label htmlFor="invite-code" className="block text-sm font-medium text-slate-700 mb-2">
              Invite Code
            </label>
            <div className="relative">
              <input
                id="invite-code"
                type="text"
                value={inviteCode}
                onChange={(e) => setInviteCode(e.target.value.toUpperCase())}
                placeholder="IZZIE-XXXXX"
                className={`w-full px-4 py-3 bg-white border rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all ${
                  inviteCodeValid === true
                    ? 'border-green-500 focus:ring-green-500'
                    : inviteCodeValid === false
                    ? 'border-red-500 focus:ring-red-500'
                    : 'border-slate-200 focus:ring-blue-500'
                }`}
                disabled={isValidatingCode || isLoading}
              />
              {/* Validation indicator */}
              <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                {isValidatingCode && (
                  <svg className="animate-spin h-5 w-5 text-slate-400" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
                )}
                {!isValidatingCode && inviteCodeValid === true && (
                  <svg className="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                )}
                {!isValidatingCode && inviteCodeValid === false && (
                  <svg className="h-5 w-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                )}
              </div>
            </div>
            {/* Validation message */}
            {inviteCodeMessage && (
              <p className={`mt-2 text-sm ${inviteCodeValid ? 'text-green-600' : 'text-red-600'}`}>
                {inviteCodeMessage}
              </p>
            )}
            <p className="mt-2 text-xs text-slate-500">
              Izzie is in private beta. Enter your invite code to get started.
            </p>
          </div>
        )}

        {/* Single Google Sign-in Button */}
        <button
          onClick={handleGoogleSignIn}
          disabled={isLoading || (REQUIRE_INVITE_CODE && !inviteCodeValid)}
          data-testid="google-signin-button"
          className="group relative w-full flex items-center justify-center gap-3 px-6 py-4 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-xl shadow-sm hover:shadow transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        >
          {isLoading ? (
            <>
              <svg
                className="animate-spin h-5 w-5 text-slate-600"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
                  className="opacity-25"
                  cx="12"
                  cy="12"
                  r="10"
                  stroke="currentColor"
                  strokeWidth="4"
                />
                <path
                  className="opacity-75"
                  fill="currentColor"
                  d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
                />
              </svg>
              <span className="text-slate-600 font-medium">Connecting...</span>
            </>
          ) : (
            <>
              {/* Google Icon */}
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path
                  fill="#4285F4"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              <span className="text-slate-700 font-medium">Sign in with Google</span>
            </>
          )}
        </button>

        {/* Trust message */}
        <p className="mt-6 text-sm text-slate-500 text-center">
          Securely connect your Gmail account
        </p>

        {/* Privacy notice */}
        <p className="mt-8 text-xs text-slate-400 text-center max-w-xs">
          By signing in, you agree to our{' '}
          <Link href="/terms" className="text-slate-500 hover:text-slate-700 underline">
            Terms of Service
          </Link>{' '}
          and{' '}
          <Link href="/privacy" className="text-slate-500 hover:text-slate-700 underline">
            Privacy Policy
          </Link>
          .
        </p>

        {/* About link */}
        <Link
          href="/about"
          className="mt-6 text-sm text-slate-500 hover:text-slate-700 transition-colors"
        >
          Learn more about Izzie
        </Link>
      </div>
    </main>
  );
}

/**
 * Landing Page (Login)
 * Beautiful, single-click OAuth experience with Google sign-in
 * Wrapped in Suspense for useSearchParams compatibility
 */
export default function HomePage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-blue-50">
          <div className="animate-pulse">
            <div className="h-8 w-32 bg-slate-200 rounded" />
          </div>
        </div>
      }
    >
      <HomePageContent />
    </Suspense>
  );
}
